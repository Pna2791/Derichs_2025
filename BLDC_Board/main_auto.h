...
void forward_command(String command) { Serial.println(command); }
...
void show_line_sensor() {
  String message =
      String(check_line_sensor(0)) + " " + String(check_line_sensor(1)) + " " +
      String(check_line_sensor(2)) + " " + String(check_line_sensor(3));
  Serial.println(message);
}
...
const float step_per_mm =
    1.0f * GEAR_RATIO * STEPS_PER_REVOLUTION / WHEEL_DIAMETER / 3.1416;
...
void auto_forward(int distance) {
  ...
  unsigned long forward_timeout =
      millis() + (unsigned long)(distance * 8 + 3000);

  while ((left_encoder.getCount() < left_pos ||
          right_encoder.getCount() < right_pos) &&
         millis() < forward_timeout) {
    ...
    // 1. HÃM TỐC NGAY LẬP TỨC xuống động cơ khi chạm mốc giảm tốc (còn 250mm)
    if (is_normal_speed && (left_encoder.getCount() > left_pos_slowdown) &&
    ...
    // 2. Dò Line trong vùng giảm tốc: Lùi check cảm biến sau (pin 39/index 3), Tiến check cảm biến trước (index 0, 1)
    if (!is_normal_speed) {
      if ((dir < 0 && check_line_sensor(3)) || 
          (dir > 0 && (check_line_sensor(0) || check_line_sensor(1)))) {
        break;
      }
    }
    ...
    if (direction != 0xFFF) {
      ...
      float delta_value =
          forward_pid.compute(target_dir + delta_angle_target, direction) /
          255 * dir;
    ...
    if (direction != 0xFFF) {
      ...
      if (abs(target_dir - direction) < error_angle) {
        ...
        Serial.println("Stoped: " +
                       String(rotate_timeout - time_out + millis()));
        return;
      }

      int rotate_speed =
          auto_rotate_speed * rotate_pid.compute(target_dir, direction) / 255;
#ifdef DEBUG
      String message =
          String(rotate_speed) + '\t' + String(target_dir - direction);
...
void open_ser2_timer(int duration_ms) {
  if (duration_ms == 2000)
    forward_command("S2_2S");
  else if (duration_ms == 1000)
    forward_command("S2_1S");
  else if (duration_ms == 3000)
    forward_command("S2_3S");
  else
    forward_command("SER2_MO");
  wait_non_blocking(duration_ms);
}

void drop_balls_sync(int count, unsigned long max_wait_ms = 2500) {
...
void chien_thuat_tha_banh() {
  ...
  // B1: Chạy đến vị trí đầu -> Mở SER 2 trong 2s rồi đóng
  ...
  // B2 & B3: SER 3 mở + DC 3 bật, đếm 3 bóng -> đóng SER 3 & tắt DC 3 (tối đa 3s)
  drop_balls_sync(3, 3000);
  if (emergency_stop)
    return;
  ...
  // B5: SER 3 mở + DC 3 bật, đếm 1 bóng -> đóng SER 3 & tắt DC 3 (tối đa 2s)
  drop_balls_sync(1, 2000);
  if (emergency_stop)
    return;
  ...
  // B7: SER 3 mở + DC 3 bật, đếm 2 bóng -> đóng SER 3 & tắt DC 3 (tối đa 2.5s)
  drop_balls_sync(2, 2500);
  if (emergency_stop)
    return;
...
void simple_strategy() {
  ...
  auto_forward(1850);
  rote_CCW();
  auto_forward(-1700); // Lùi 1.7m: tự hãm tốc và dừng khi nhận Line 4 (hoặc hết quãng đường nếu sensor lỗi)
