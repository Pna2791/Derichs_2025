...
- **Quy trình bắt buộc khi muốn Xóa / Tái cấu trúc (Refactor)**: Nếu phát hiện mã nguồn cũ dư thừa, lỗi thời hoặc có giải pháp thay thế tốt hơn, AI **BẮT BUỘC** phải lập bản phân tích chi tiết và trình bày cho người dùng trước khi xóa:
  1. **Xóa để làm gì?**: Lý do kỹ thuật cụ thể, các điểm nghẽn hoặc rủi ro của mã cũ.
  2. **Sau khi xóa thì có gì tốt hơn?**: Lợi ích rõ ràng về hiệu năng (tần số quét Hz), độ an toàn cơ khí, giải phóng RAM/ROM hay tính đồng bộ.
  3. **Kế hoạch dự phòng**: Đảm bảo không làm gãy các module phụ thuộc khác.
  *(Chỉ thực hiện xóa sau khi người dùng bấm xác nhận đồng ý).*
...
- **Định chuẩn Đơn vị Góc IMU**: Cảm biến Hi229 xuất góc đơn vị $0.1^\circ$ (dải $-1800 \dots +1800$). Mọi phép tính sai số góc luôn phải qua hàm `standard_dir(target, direction)` để xử lý bước nhảy qua mốc $\pm 180^\circ$.
